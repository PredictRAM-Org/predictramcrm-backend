...
module.exports = class MarketCallController {
  static async createMarketCall(req, res, next) {
    try {
      ...
      await session.withTransaction(async (session) => {
        ...
        // create market call
        const [marketCall] = await MarketCallService.createMarketCall(
          {
            title,
            description,
            // clients,
        ...
        // create stocks
        const stockPayload = stockList?.map((stock) => ({
          ...stock,
          organization,
          marketCall: marketCall?._id,
        }));

        const stocks = await StockService.createStock(stockPayload, {
          session,
        });

        // create add stock history
        const callHistoryPayload = {
          marketCall: marketCall?._id,
          currentPortfolio: stocks,
          date: new Date(),
          stockChanges: stocks.map((stock) => ({
            symbol: stock?.symbol,
            state: STOCK_CHANGE_STATE.ADD,
            date: new Date(),
            currentStockInfo: { ...stock.toObject() },
          })),
          organization,
        };

        await MarketCallHistoryService.createMarketCallHistory(
          callHistoryPayload,
          { session }
        );
      });
      res.json(response({}, "market call created successfully"));
    } catch (err) {
      next(err);
    }
  }
